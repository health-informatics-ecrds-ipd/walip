...
if(in_array($zone_sante,["DKO", "DKC", "DKN", "DKS"])) {
    ...
    if(($cas_suspect !== "1") && ($updateNoTerrain)){
        mysqli_query($link, "update cnrfj set no_terrain = NULL where numbECRDS = ". $numbECRDS );
        //setNoEpid($link, $lien_cas_contact, $numbECRDS);
    }      
      
    mysqli_query($link, "update cnrfj set age_annees = round(DATEDIFF(CURDATE(), date_nais) / 365.25) where numbECRDS = ".$numbECRDS." and  date_nais is not null");
...
//-----------------------------------------------------------------------------------------------------------------//
//---------If callback is from others (out of DKO, DKN, DKS, DKC) districts then scale up with numbECRDS ---------//
else {
