...
function fillTable(tableid, start, end, number){
	 ...
	 $.post("posts/post_get_questionnaire.php", {'start':start, 'end':end}, function(data){
		 ...
		 $.each(data, function(k,v){
			 var label = number==true? (i+'.' + v['question']) :  v['question'];
...
$(document).ready(function() {
	fillTable("cap_form0", 62, 6, false);
	fillTable("cap_form1", 1, 8, true);
	fillTable("cap_form2", 9, 7, true);
	fillTable("cap_form3", 16, 8, true);
	fillTable("cap_form4", 24, 14, true);
	fillTable("cap_form5", 38, 24, true);
